...
export default async function handler(req,res){
  ...
  if(req.method==='GET'){
    const {data,error}=await ctx.userClient.rpc('admin_list_users');
  ...
  if(action==='status'){
    ...
    const {error}=await ctx.userClient.rpc('admin_set_status',{p_user:userId,p_status:status});
    if(error) return json(res,400,{error:error.message});
    return json(res,200,{ok:true});
  }
  if(action==='permissions'){
    const permissions=Array.isArray(req.body?.permissions)?req.body.permissions:[];
    const {error}=await ctx.userClient.rpc('admin_set_permissions',{p_user:userId,p_permissions:permissions});
    if(error) return json(res,400,{error:error.message});
    return json(res,200,{ok:true});
  }
