...
export default async function handler(req,res){
  ...
  if(req.method==='GET'){
    const {data,error}=await ctx.userClient.rpc('admin_my_profile');
  ...
  const {error}=await ctx.userClient.rpc('admin_update_profile',{p_display_name:displayName,p_phone:phone});
