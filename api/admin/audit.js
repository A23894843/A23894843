...
export default async function handler(req,res){
  ...
  const {data,error}=await ctx.client.from('audit_logs').select('id,admin_id,action,target_type,target_id,metadata,created_at').order('created_at',{ascending:false}).limit(200);
