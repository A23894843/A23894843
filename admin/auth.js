/* =========================================================
   ...
   Session policy:
   - Session survives page refresh/browser restart while Supabase refresh token is valid.
   - Admin session is automatically signed out after 30 minutes of inactivity.
   - Activity is shared across tabs through localStorage.
   - /admin/config.js remains untouched.
========================================================= */
let sb=null;

const ADMIN_INACTIVITY_TIMEOUT_MS = 30 * 60 * 1000;
const ADMIN_ACTIVITY_KEY = 'abhinandan_admin_last_activity';
const ADMIN_TIMEOUT_CHECK_MS = 60 * 1000;
let sessionWatchStarted = false;
let activityTimer = null;

function setMessage(text,type='error'){
  const el=document.getElementById('message')||document.getElementById('msg');
  if(!el)return;
  el.textContent=text;
  ...
  if(!sb){
    sb=window.supabase.createClient(SUPABASE_URL,SUPABASE_ANON_KEY,{
      auth:{
        ...
        storageKey:'abhinandan-admin-auth'
      }
    });
  }
  startSessionWatch();
...
async function enforceTimeout(){
  ...
  if(last && Date.now()-last >= ADMIN_INACTIVITY_TIMEOUT_MS){
    localStorage.removeItem(ADMIN_ACTIVITY_KEY);
    await logout(true,'timeout');
  }
}
...
function startSessionWatch(){
  ...
  activityEvents.forEach(event=>window.addEventListener(event,touchActivity,{passive:true}));

  const existing=getLastActivity();
  if(!existing || Date.now()-existing >= ADMIN_INACTIVITY_TIMEOUT_MS){
    localStorage.setItem(ADMIN_ACTIVITY_KEY,String(Date.now()));
  }

  window.setInterval(enforceTimeout,ADMIN_TIMEOUT_CHECK_MS);
  ...
  window.addEventListener('storage',event=>{
    if(event.key===ADMIN_ACTIVITY_KEY && event.newValue){return;}
    if(event.key==='abhinandan_admin_force_logout'){
      window.location.href='/admin/login.html?reason=logout';
    }
  });
}
...
async function login(email,password,turnstileToken){
  ...
  try{
    ...
    if(profile.data.status!=='approved'||!['owner','admin','support'].includes(profile.data.role)){
      await sb.auth.signOut();
      throw new Error('Account is pending, disabled, or not authorized.');
    }
    localStorage.setItem(ADMIN_ACTIVITY_KEY,String(Date.now()));
...
async function signup(name,email,password,confirmPassword,turnstileToken){
  ...
  try{
    ...
    setMessage('Registration submitted. Verify your email if required, then wait for admin approval.','success');
    return true;
...
async function guard(){
  ...
  if(!ctx){await logout(false);return false;}
  return true;
}
...
async function logout(redirect=true,reason='logout'){
  try{
    if(!sb)init();
    localStorage.removeItem(ADMIN_ACTIVITY_KEY);
    localStorage.setItem('abhinandan_admin_force_logout',String(Date.now()));
    if(sb)await sb.auth.signOut();
  }finally{
    if(redirect)window.location.href=`/admin/login.html?reason=${encodeURIComponent(reason)}`;
  }
}

async function docs(){
  if(!init())throw new Error('Supabase is not configured.');
  const r=await sb.from('documents').select('*').order('created_at',{ascending:false});
...
async function uploadDoc(title,type,file,downloadAllowed=false,visibility='public'){
  ...
  if(visibility==='private')downloadAllowed=false;
  if(!['cv','report','certificate'].includes(type))throw new Error('Invalid document type.');
  ...
  try{
    ...
    if(!u.data?.user)throw new Error('Authentication session expired.');
    const r=await sb.from('documents').insert({title:title.trim(),type,storage_path:path,download_allowed:Boolean(downloadAllowed),visibility,created_by:u.data.user.id});
    if(r.error)throw r.error;
    return true;
  }catch(e){await sb.storage.from('documents').remove([path]);throw e;}
}

async function getAdminDocumentPreviewUrl(path){
  if(!init())throw new Error('Supabase is not configured.');
  const r=await sb.storage.from('documents').createSignedUrl(path,300);
  if(r.error)throw r.error;
  return r.data.signedUrl;
}

async function updateDocAccess(id,downloadAllowed){
  if(!init())throw new Error('Supabase is not configured.');
  const r=await sb.from('documents').update({download_allowed:Boolean(downloadAllowed)}).eq('id',id);
  if(r.error)throw r.error;
  return true;
}
...
async function deleteDoc(id,path){
  if(!init())throw new Error('Supabase is not configured.');
  const r=await sb.from('documents').delete().eq('id',id);
  if(r.error)throw r.error;
  if(path){const s=await sb.storage.from('documents').remove([path]);if(s.error)throw s.error;}
  return true;
}

window.verifyCaptcha=verifyCaptcha;
window.getAdminContext=getAdminContext;
window.requirePermission=requirePermission;
window.guard=guard;
window.logout=logout;
window.docs=docs;
window.uploadDoc=uploadDoc;
window.getAdminDocumentPreviewUrl=getAdminDocumentPreviewUrl;
window.updateDocAccess=updateDocAccess;
window.deleteDoc=deleteDoc;
window.ADMIN_INACTIVITY_TIMEOUT_MS=ADMIN_INACTIVITY_TIMEOUT_MS;
